...
function initStartPage() {
    ...
    if (uploadForm) {
        uploadForm.removeEventListener("submit", handleVideoUpload); // Duplikate verhindern
            ...
            card.innerHTML = `
                ...
                <div class="video-info" style="padding: 10px;">
                    ...
                    <div class="video-meta" style="display: flex; justify-content: space-between; align-items: center; font-size: 11px; color: #909090;">
                        ...
                        <span class="badge ${video.origin === "App 1" ? "app1" : ""}">${video.origin}</span>
                    </div>
                </div>
            `;
...
async function showSingleVideo(videoId) {
    ...
    try {
        ...
        badge.className = `badge ${video.origin === "App 1" ? "app1" : ""}`;
        ...
        async function loadComments() {
            try {
                ...
                comments.forEach(c => {
                    ...
                    div.style.borderLeft = "4px solid #CFD11A";
...
function initHlsPlayer(videoElement, streamUrl) {
    ...
    if (Hls.isSupported()) {
        ...
        hls.on(Hls.Events.MANIFEST_PARSED, function () {
            ...
            // hls.levels enthält die Auflösungsstufen (z.B. 360p, 720p, 1080p) aus der master.m3u8
        ...
        // Event: Wenn der Nutzer die Auflösung manuell ändert
        qualitySelect.onchange = () => {
            const levelId = parseInt(qualitySelect.value);
            hls.currentLevel = levelId; // -1 schaltet zurück auf automatisches
            console.log(`Qualität manuell geändert auf: ${levelId === -1 ? "Auto" : hls.levels[levelId].height + "p"}`);
        };

    } else if (videoElement.canPlayType("application/vnd.apple.mpegurl")) {
        // Native Native Unterstützung für Safari / Apple Ecosystem
...
async function handleVideoUpload(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            statusMessage.innerText = "done " + result.message;
